 ...
 * @FilePath: /uni-preset-vue-vite-ts 2/src/pages/areaManagement/areaManagement/components/areaConfig.vue
 * @描述: 片区配置
 * 
-->
<template>
  <view class="wrap">
    <view class="main">
      ...
      >
        <!-- 扫码支付 -->
        <template #paymentHeaderRight>
          <u-switch v-model="paymentValue" ></u-switch>
        </template>

        <!-- 支付页面消息制定 -->
        <template #pageCustomSlot>
          <view class="pageCustom" @click="pageCustomClick">
            <text class="title">未设置</text>
...
// 默认分账方式
const clearingConfigChange = (val: any, key: string) => {
  let param = {
  ...
  areaOrgUpdateMode(param).then((res) => {
    let { data } = res
  });
}

const productTypeChange11 = (val: any, key: string) => {
...
const getAccountMethod =()=>{
  ...
  clearingAccountChargeConfigList(param).then((res)=>{
    ...
    isSetData('clearingConfigName', data.clearingConfigDtos, 0)
  })
}
